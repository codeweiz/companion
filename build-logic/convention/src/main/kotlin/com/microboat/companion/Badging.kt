...
@DisableCachingByDefault(because = "Writes the golden badging file into the source tree")
abstract class UpdateBadgingTask : DefaultTask() {
    ...
    // Declaring only the golden file as an output (instead of copying into the whole project
    // directory) keeps this task's output narrow, so unrelated tasks that read from the project
    // directory are not treated as consumers of this task's output.
    @get:OutputFile
    abstract val goldenBadging: RegularFileProperty

    @TaskAction
    fun taskAction() {
        generatedBadging.get().asFile.copyTo(
            target = goldenBadging.get().asFile,
            overwrite = true,
        )
    }
}
